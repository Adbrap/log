...
def Finder_IETE(time1, time_name1, start1):
    ...
    with my_lock:
        try:
            ...
            remplacement('%log%',f'{datetime.datetime.now().replace(second=0, microsecond=0)} PROBLEME DE CONNECTION AU TITRE ET (OU) DE RESULTAT SUR: {ticker} {time1} {time_name1}\n%log%','Log.txt', f'Log.txt')
            Write.Print("<!!> <!!> <!!> <!!> ERREUR CRITIQUE <!!> <!!> <!!> <!!>", Colors.yellow, interval=0.000)
    ...
    try:
        ...
        Write.Print("<!!> <!!> <!!> <!!> PAS ASSEZ DE DONNEES <!!> <!!> <!!> <!!>", Colors.red, interval=0.000)
    # ----- initialisation des pointeurs de la figure -----#
    if passed == True:
        ...
        # ----- condition pour garantir la forme de l'iete  -----#
            ...
            # ----- regarde si la target a deja ete toucher en volatilité avant affichage  -----#
                ...
                #        fig.patch.set_alpha(0.3)
                #        remplacement('%log%',f'{datetime.datetime.now().replace(second=0, microsecond=0)} PROBLEME DANS LE PASSAGE ORDRE IB SUR: {ticker} {time1} {time_name1}\n%log%','Log.txt', f'Log.txt')
                ...
                try:
                    ...
                    remplacement('%log%',f'{datetime.datetime.now().replace(second=0, microsecond=0)} PROBLEME D\'AFFICHAGE MATPLOTLIB SUR: {ticker} {time1} {time_name1}\n%log%','Log.txt', f'Log.txt')
                    Write.Print("<!!> <!!> <!!> <!!> ERREUR CRITIQUE <!!> <!!> <!!> <!!>", Colors.red, interval=0.000)
